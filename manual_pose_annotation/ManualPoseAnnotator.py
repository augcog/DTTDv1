...
class ManualPoseAnnotator:
    ...
    def icp_pose_initializer(rgb, depth, depth_scale, camera_intrinsic_matrix, camera_distortion_coefficients, objects):
        ...
        print('Please select the approximate center of the object in the scene')
        plt.imshow(rgb)
        p1, p2 = plt.ginput(2)
        plt.close()
            

        return np.array([np.eye(4) for i in range(len(objects))])
    ...
    and returns a numpy array of size (Ox4x4), the affine transform for each object
    """
    def annotate_pose(self, rgb, depth, depth_scale, initialization_method=None):
        if initialization_method:
            ...
            initial_poses = np.array([np.eye(4) for i in range(len(self.objects))])
